...
- 每条事件支持图片：新建／编辑时选择多图、移除并保存，详情弹窗显示缩略图，点击看大图、切换及放大。支持 JPEG、PNG、WebP、HEIC／HEIF；每事件最多12张，单张20 MiB。图片与缩略图存入 SQLite，随事件删除并自动纳入现有备份。实现、接口与验收见 [事件图片](docs/EVENT_IMAGES.md)。

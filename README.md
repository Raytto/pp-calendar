...
- 仅支持按天记录的月视图；支持前后翻月、回到今天、触控左右滑动，并以快速的方向动画提示当前翻页方向；系统启用“减少动态效果”时自动停用动画。
...
- 点击任意日期创建事件；可查看、编辑、删除标题、日期、所属日历和备注。桌面端可直接把事件拖到任意日期改期，拖到月视图左右边缘并短暂停留会自动翻月后继续拖放。
- 新建／编辑弹窗采用固定标题栏与底部操作栏，中间表单独立滚动；小屏、横屏及软键盘适配见 [事件编辑弹窗](#事件编辑弹窗)。
- 每条事件支持图片：新建／编辑时点击、拖放或粘贴添加多图，卡片预览、移除及撤销后保存；详情提供图片管理入口，大图支持缩略图导航、切换及放大。支持 JPEG、PNG、WebP、HEIC／HEIF；每事件最多12张，单张20 MiB。图片与缩略图存入 SQLite，随事件删除并自动纳入现有备份。实现、接口与验收见 [事件图片](docs/EVENT_IMAGES.md)。
...
- 新建事件默认沿用当前浏览器上次成功创建事件的所属日历，刷新后仍保留；取消、创建失败或编辑旧事件不更新此默认值。无记录或对应日历已删除时回退到列表首项；事件文字已创建但图片上传失败时仍记住所属日历。
...
- PPHK 宿主管理任务可使用 `python -m app.agent_cli` 调用现有日历／图片 API，无需长期保存浏览器 Cookie；仅限 root 本机使用。饮食内容、热量和营养由日历 skill 写入普通备注，不另设营养表。

## 事件编辑弹窗

- 新建与编辑共用三段布局：标题／关闭、中间表单滚动区、底部操作栏。标题、日期、日历、备注、图片及错误提示都在中间区域；保存、取消和编辑时的删除按钮不参与滚动，操作栏背景不透明并高于表单内容，按钮最小高度44px。
- 弹窗高度限制在当前可见视口以内，内容区域可缩小，手机底部兼容安全区。监听 `visualViewport` 的高度和偏移变化，适配软键盘缩小／平移可见区域的情况；双指缩放期间不追随视口改变，保留用户缩放操作。移动端输入字号至少16px，避免聚焦时自动放大。
- 重开编辑器回到表单顶部；必填校验可滚动定位字段，保存失败会将错误提示滚入内容区，底部操作栏位置不变。
- 2026-09-21 验证：7种桌面／手机／横屏／矮屏尺寸（最小320px宽、240px高）下深浅主题均检查滚动前后操作栏位置和按钮命中；另模拟键盘将可见区域压缩为340px并向下偏移75px。共33项布局／状态检查、创建／编辑／取消／错误重试／图片上传通过，页面脚本错误0，现有21项自动测试通过。软键盘为视口模拟，未做真实iPhone Safari验收。静态版本 `styles.css?v=20`、`app.js?v=29`。
